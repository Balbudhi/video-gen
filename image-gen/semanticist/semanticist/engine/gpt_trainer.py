...
class GPTTrainer(nn.Module):
    def __init__(
        ...
        eval_classes=[
            ...
            404,
            850,
        ],  # goldfish, cock, tiger cat, hourglass, ship, golden retriever, husky, race car, airliner, teddy bear
        blr=1e-4,
        cosine_lr=False,
        lr_min=0,
        warmup_epochs=100,
        ...
        assert (
            ae_model_path.endswith(".safetensors")
            or ae_model_path.endswith(".pt")
            or ae_model_path.endswith(".pth")
            or ae_model_path.endswith(".pkl")
        )
        assert osp.exists(ae_model_path), (
            f"AE model checkpoint {ae_model_path} does not exist"
        )
        ...
        if not test_only:
            ...
            self.g_optim = create_optimizer(
                self.gpt_model, weight_decay=0.05, learning_rate=lr
            )
        ...
        if (
            ...
            self.ema_model = EMAModel(
                self.accelerator.unwrap_model(self.gpt_model), self.device
            )
    ...
    def _build_cache(self):
        ...
        print(
            f"Rank {rank}: Creating new cache with {num_augs} augmentations per image..."
        )
        ...
        targets_mmap = np.memmap(
            targets_file, dtype="int64", mode="w+", shape=(shard_size * num_augs,)
        )
        ...
        with torch.no_grad():
            for i, batch in enumerate(
                ...
                if len(imgs.shape) == 5:  # [B, num_augs, C, H, W]
                    ...
                    targets = (
                        targets.unsqueeze(1).expand(-1, A).reshape(-1)
                    )  # [B*num_augs]
                ...
                for split_idx, (img_split, targets_split) in enumerate(
                    zip(imgs_splits, targets_splits)
                ):
                    img_split = img_split.to(self.device, non_blocking=True)
                    slots_split = self.ae_model.encode_slots(img_split)[
                        :, : self.train_num_slots, :
                    ]
        ...
        self.cached_targets = np.memmap(
            targets_file, dtype="int64", mode="r", shape=(shard_size * num_augs,)
        )
        ...
        with self.accelerator.accumulate(self.gpt_model):
            ...
            if self.accelerator.sync_gradients and self.max_grad_norm is not None:
                self.accelerator.clip_grad_norm_(
                    self.gpt_model.parameters(), self.max_grad_norm
                )
            self.g_optim.step()
    ...
    def _handle_periodic_ops(self, loss, logger):
        ...
        if (self.steps % self.sample_every == 0) or (
            self.eval_fid and self.steps % self.fid_every == 0
        ):
            empty_cache()
            self.evaluate()
            self.accelerator.wait_for_everyone()
    ...
    def evaluate(self, use_ema=True):
        ...
        if not self.test_only:
            ...
            with self._autocast_ctx():
                slots = generate(
                    unwraped_gpt_model,
                    classes,
                    self.num_slots_to_gen,
                    cfg_scale=self.cfg,
                    cfg_schedule=self.cfg_schedule,
                    temperature=self.temperature,
                )
                if self.num_slots_to_gen < self.num_slots:
                    null_slots = self.ae_model.dit.null_cond.expand(
                        slots.shape[0], -1, -1
                    )
                    null_slots = null_slots[:, self.num_slots_to_gen :, :]
                    slots = torch.cat([slots, null_slots], dim=1)
                imgs = self.ae_model.sample(
                    slots, targets=classes, cfg=self.ae_cfg
                )  # targets are not used for now
            ...
            if self.accelerator.is_main_process:
                save_image(
                    ...
                    os.path.join(
                        ...
                        f"step{self.steps}_aecfg-{self.ae_cfg}_cfg-{self.cfg_schedule}-{self.cfg}_slots{self.num_slots_to_gen}_temp{self.temperature}.jpg",
                    ),
                )
        if self.eval_fid and (self.test_only or (self.steps % self.fid_every == 0)):
            ...
            save_folder = os.path.join(
                self.image_saved_dir,
                f"gen_step{self.steps}_aecfg-{self.ae_cfg}_cfg-{self.cfg_schedule}-{self.cfg}_slots{self.num_slots_to_gen}_temp{self.temperature}",
            )
            if self.accelerator.is_main_process:
            ...
            padding_size = world_size * batch_size - (
                total_images % (world_size * batch_size)
            )
            ...
            if self.accelerator.is_main_process:
                print(
                    f"Generating {total_images} images ({images_per_class} per class) across {world_size} GPUs"
                )
            ...
            for i in range(local_num_steps):
                ...
                with torch.no_grad():
                    with self._autocast_ctx():
                        ...
                        if self.num_slots_to_gen < self.num_slots:
                            ...
                            slots = torch.cat([slots, null_slots], dim=1)
                        imgs = self.ae_model.sample(
                            slots, targets=labels, cfg=self.ae_cfg
                        )

                samples_in_batch = min(
                    batch_size * world_size, total_images - gen_img_cnt
                )
                ...
                if self.accelerator.is_main_process and i % 10 == 0:
                    print(
                        f"Avg generation time: {used_time / gen_img_cnt:.5f} sec/image"
                    )
            ...
            if self.accelerator.is_main_process:
                ...
                print(
                    f"Generated {generated_files} images out of {total_images} expected"
                )
                ...
                self.accelerator.log(
                    {
                        ...
                        "cfg_schedule": self.cfg_schedule,
                        "temperature": self.temperature,
                        "num_slots": self.test_num_slots
                        if self.test_num_slots is not None
                        else self.train_num_slots,
                    },
                    step=self.steps,
                )
